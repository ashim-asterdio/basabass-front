...
const PropertyDetails: NextPage = () => {
  interface multiValuesType {
    unit: string;
    varients: { carpetArea: string; price: string }[];
  }
  ...
  const [multiValues, setMultiValues] = useState<multiValuesType[]>([
    { unit: "", varients: [{ carpetArea: "", price: "" }] },
  ]);
  ...
  const check = (error: any, param: string, index: number, indx: number) => {
    if (param == "unit") {
      ...
      if (
        error &&
        error[index] != undefined &&
        error[index]?.varients != undefined &&
        error[index]?.varients[indx]?.carpetArea != undefined &&
        error[index]?.varients[indx]?.carpetArea
      )
        return (
      ...
      if (
        ...
        error[index] != undefined &&
        error[index]?.varients != undefined &&
        error[index]?.varients[indx]?.price != undefined &&
        error[index]?.varients[indx]?.price
      )
        return (
          <span className={style.error}>
            {error[index]?.varients[indx]?.price}
  ...
  let temp: {
    name: string;
    info: [{ carpetArea: string; carpetPrice: string }];
  }[] = [];
  ...
  const { values, errors, touched, handleSubmit, handleChange } = useFormik({
    ...
    onSubmit: async (values, formikHelpers) => {
      ...
      details.multiple_units = values?.multipleValue;
      console.log("temp",temp)
      values.multipleValue.map((value:multiValuesType,index:number)=>{
        // temp.append({name:value.unit,info:value.varients})
        console.log("temp2",temp)
      })
      console.log("temp2",temp)
    //   for (let i: number = 0; i < values.multipleValue.length; i++) {
    //     temp[i]?.name=details?.multiple_units[i]?.unit
    //   }
      ...
      >
        <div className={style.propertydetails_container}>
          ...
          <div
            ...
            style={{ display: rollBack.category == "land" ? "none" : "flex" }}
          >
            <label className={style.labelTitle}>
              Building Details
              <a href="#" style={{ display: "flex" }}>
          ...
          <div className={style.multipleUnitsComponent}>
            ...
            <div className={style.multipleUnits}>
              <div className={style.multipleUnitContent}>
                <div className={style.multipleUnitSelect}>
                  <div className={style.radioDiv}>
                    <SmallRadio
                      ...
                      name="numberOFUnits"
                      onChange={handleChange}
                      otherValue={values.numberOFUnits}
                    />
                    <SmallRadio
                      value="false"
                      toShow="No"
                  ...
                  <button
                    className={style.addUnitButton}
                    onClick={addUnit}
                    style={{
                      display: values.numberOFUnits == "true" ? "flex" : "none",
                    }}
                  >
                    Add Unit
                  </button>
                ...
                <div
                  className={style.unitsContainer}
                  style={{
                    display: values.numberOFUnits == "true" ? "flex" : "none",
                  }}
                >
                  ...
                  {multiValues.map(
                    (content: multiValuesType, index: number) => {
          ...
          <div className={style.amenitiesComponent}>
            ...
            <div className={style.aminitiesDiv}>
              ...
              {list?.map((value: any, key) => (
                <AmenitiesCheckbox
                  value={value._id}
                  otherValue={values.amenities}
                  onChange={handleChange}
                  key={key}
                  holder={value.amenity}
                />
              ))}
            </div>
